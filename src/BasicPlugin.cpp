...
public:
    HRESULT VDJ_API OnLoad() override {
        if (FAILED(DeclareParameterButton(&nextPageButton_, 1, "Next lyrics page", "Next")) ||
            FAILED(DeclareParameterButton(&previousPageButton_, 2, "Previous lyrics page", "Prev"))) return E_FAIL;
        return S_OK;
    }
    ...
    HRESULT VDJ_API OnGetPluginInfo(TVdjPluginInfo8* info) override {
        ...
        info->Flags = VDJFLAG_PROCESSLAST | VDJFLAG_VIDEO_OVERLAY | VDJFLAG_VIDEO_OUTPUTRESOLUTION;
#ifdef EMBEDDED_LYRICS_MASTER
    ...
    HRESULT VDJ_API OnDeviceInit() override {
        ...
        return texture_.Initialize(device_) && renderer_.Initialize(device_) ? S_OK : E_FAIL;
    }
...
private:
    ...
    int PluginDeck() {
        double deck = 0.0;
        return SUCCEEDED(GetInfo("get_plugindeck", &deck)) ? static_cast<int>(deck) : 0;
    }
#else
    int VisibleVideoDeck() {
        double crossfader = 0.0, leftDeck = 1.0, rightDeck = 2.0;
        GetInfo("video_crossfader", &crossfader);
        GetInfo("get_leftdeck", &leftDeck);
        GetInfo("get_rightdeck", &rightDeck);
        return static_cast<int>(crossfader < 0.5 ? leftDeck : rightDeck);
    }
#endif
