...
- **Embedded Lyrics Deck** runs independently on each deck. This is the
  recommended variant when VirtualDJ controls video transitions automatically.
- **Embedded Lyrics Master** draws over the final master output and selects the
  left/right source from `video_crossfader`.
...
- Smooth timestamp-to-timestamp vertical scrolling for synchronized lyrics.
- Independent per-deck timing through `get_plugindeck` in the deck variant.
- Selection of the visible left/right video deck using `video_crossfader` in
  the master variant.
